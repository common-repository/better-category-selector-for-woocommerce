<?php
/**
 * Better Category Selector for WooCommerce Library Class
 */
...
class HD_BCS_Lib {
	...
	const DOMAIN = 'better-category-selector-for-woocommerce';
	 ...
	 * @since 1.5.0
	 */
	public static $settings_key = 'HD_BCS_options';

	/**
	 * The current plugin settings.
	 * @see   HD_BCS_Lib::set_settings()
	 * @var   array
	 * @since 1.5.0
	 */
	 ...
	 * @see   HD_BCS_Lib::get_default_settings()
	 * @var   array
	 * @since 1.5.0
	 */
	...
	public static function add_settings_link( $links, $file ) {
		static $this_plugin;
		if ( ! $this_plugin ) {
			$this_plugin = HD_BCS_BASENAME;
		}

		if ( $file === $this_plugin ) {
			$settings_link = '<a href="' . admin_url( 'options-general.php?page=bcs-settings' ) . '">' . esc_attr__( 'Settings', self::DOMAIN ) . '</a>';
	...
	public static function admin_footer() {
		$plugin_data = get_plugin_data( HD_BCS_FILE );
		echo esc_html($plugin_data['Title']) // @codingStandardsIgnoreLine >> no valid esc function.
			. ' | ' . esc_attr__( 'Version', HD_BCS_Lib::DOMAIN ) . ' ' . esc_html( $plugin_data['Version'] )
			. ' | ' . esc_html($plugin_data['Author']) // @codingStandardsIgnoreLine >> no valid esc function.
			. ' | <a href="http://no.brainerapps.com">NO.BrainerAPPs / HisDesigns LLC</a>'
			. ' | <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=hisdesigns@hisdesigns.org">' . esc_attr__( 'Donate', self::DOMAIN ) . '</a>
		<br />';
	}
	...
	public static function install_plugin() {
		...
		if ( get_option( 'HD_BCS_db_version' ) ) {
			$installed_version = get_option( 'HD_BCS_db_version' );
		} else {
			$installed_version = 0;
		}

		// check if the db version is the same as the db version constant
		if ( (string) HD_BCS_DB_VERSION !== (string) $installed_version ) {
			// update options This isn't needed until
      // we add an options page.
			// self::update( $installed_version );
			update_option( 'HD_BCS_db_version', HD_BCS_DB_VERSION );
		}
	...
	private static function update( $version ) {
		...
		if ( empty( $settings ) ) {
			...
			add_option( 'HD_BCS_db_version', HD_BCS_DB_VERSION );
			return;
		}
	...
	public static function set_settings( $settings, $key = null ) {
		 ...
		 *                                          See HD_BCS_Lib::parse_defined_classes().
